...
GI_2000_intep_nominize_ajusted=cell(1,31);
...
for mm=1%:5 仅取max变量
    ...
    for kk=1:31
        ...
        idx = sub2ind(size(data_crop),province_crop_id(:,1),province_crop_id(:,2)); % row/con to index
        GI_province=GI_2000_intep_nominize{kk}(:,mm);
...
%       %% weigh settings
%     weight_province=GI_2000_weight_all{kk};
...
%         +data_aridity_province/10*weigth_opt(4);
     ...
     GI_2000_intep_nominize_ajusted{kk}(:,mm)=GI_province_ajusted;
    end
end
%%
output_fold='F:\Data_ZL\MODIS\';
save ([output_fold,num2str(year_map),'\GI_2000_intep_nominize_ajusted.mat'], 'GI_2000_intep_nominize_ajusted','-v7.3');
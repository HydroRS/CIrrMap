   ...
   for kk=1:length(Nereast_Irr30_in_Irr250m_lat)
      if rem(kk,1000000)==0
        ...
        current_id_lat=Nereast_Irr30_in_Irr250m_lat(kk);
         ...
         result(kk)=min(length(data_point(data_point>41&data_point<50))/69,1);
         
   end
  %保存被建设用地、水域和未利用地覆盖的网格
...
geotiffwrite([output_local,num2str(year_map),'\2000new3_water.tif'],simu_250m,Ref, 'GeoKeyDirectoryTag', data_GeoInfor_base.GeoTIFFTags.GeoKeyDirectoryTag);

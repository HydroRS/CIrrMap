...
%weight_opt_all=EVI_2000_weight_all.EVI_2000_weight_all;
...
for kk=1:31
   ...
   EVI_province=EVI_2000_intep{kk}(:,1);
     EVI_province_neighbor=EVI_2000_intep_nominize{kk}(:,1);
    ...
    proportion_province=data_crop(idx);
     ...
     EVI_province_ajusted=temp.*EVI_province;
      EVI_province_neighbor_ajusted=temp.*EVI_province_neighbor;
...
%             +data_aridity_province/10*0.25;
        ...
        for jj=1:length(current_provinc_county_Irr);
            ...
            if  delta_dem<=100&&ismember(kk,NCP)==1 %华北平原平原县
                max_sutability=EVI_province_neighbor_ajusted(county_in_province);
            else
                county_temp_sutability=sutability(county_in_province);
                county_temp_GI=EVI_province_ajusted(county_in_province);
...
% EVI_2000_weight_all=weight_opt_all;
% save ([output_fold,num2str(year_map),'\EVI_2000_weight_all.mat'], 'EVI_2000_weight_all','-v7.3');

% output_local='F:\Data_ZL\IrrMap\';
% geotiffwrite([output_local,num2str(year_map),'\county_level\test06\EVI_sutability_threshold_IrrArea_opt918_V9.tif'],sutability_threshod_IrrArea,Ref, 'GeoKeyDirectoryTag', data_GeoInfor.GeoTIFFTags.GeoKeyDirectoryTag);

    ...
    for jj=1:length(current_provinc_county_Irr);
         ...
         Cecsus_IrrArea=current_provinc_county_Irr(jj)*1e7;   % 1000 hm2 to m2
            ...
            if  delta_dem<=100&&ismember(kk,NCP)==1 %华北平原平原县
                ...
                county_temp_NDVI=NDVI_province_ajusted(county_in_province);
                max_sutability=county_temp_sutability.*county_temp_NDVI;
            end
...
geotiffwrite([output_local,num2str(year_map),'\county_level\test09\NDVI_sutability_threshold_IrrArea_opt_act.tif'],sutability_threshod_IrrArea,Ref, 'GeoKeyDirectoryTag', data_GeoInfor.GeoTIFFTags.GeoKeyDirectoryTag);

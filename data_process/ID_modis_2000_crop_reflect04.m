...
modis_refct04_crop_id=cell(1,31);
for kk=1:31
    
    kk
    current_province_id=province_crop_id_2000{kk};
    if kk==1
        data=imread([land_loc,num2str(year_map),'\',['MOD09A1.006_sur_refl_b04_doy', num2str(year_map),'049','_aid0001.tif']]);
        data_GeoInfor=geotiffinfo([land_loc,num2str(year_map),'\',['MOD09A1.006_sur_refl_b04_doy', num2str(year_map),'049','_aid0001.tif']]);
...
%         data_GeoInfor=geotiffinfo([land_loc,num2str(year_map),'\',['MOD09Q1.006_sur_refl_b02_doy', num2str(year_map),'049','_aid0001.tif']]);
    ...
    data=imread([land_loc,num2str(year_map),'\albedo\',['MCD43A3.006_Albedo_WSA_shortwave_doy', num2str(year_map),'055','_aid0001.tif']]);
        data_GeoInfor=geotiffinfo([land_loc,num2str(year_map),'\albedo\',['MCD43A3.006_Albedo_WSA_shortwave_doy', num2str(year_map),'055','_aid0001.tif']]);
    
  
        [row, col]=find(zeros(size(data))<10e15);
    ...
    Nereast_crop_in_modis= knnsearch(current_modis_row_col(:,2:3),current_province_id(:,3:4));
    
    % find the crop location in original moidis polygon
    current_province_modis=current_modis_row_col(Nereast_crop_in_modis(:,1));
    
    modis_refct04_crop_id{kk}=current_province_modis;
    
end

%%
save([ID_loc,num2str(year_map),'\modis_refct04_crop_id.mat'],'modis_refct04_crop_id','-v7.3');
% save('modis_refct04_crop_id.mat','modis_refct04_crop_id','-v7.3');

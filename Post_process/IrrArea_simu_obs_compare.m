...
CIrrMap= geotiffread([land_loc,num2str(year_map),'\county_level\test08\2000_Irr_map_county_China_final.tif']);
...
%            idx = sub2ind(size(data_crop),current_province_crop_id(crop_current_county,1),current_province_crop_id(crop_current_county,2)); % row/con to index
           crop_proportion_current_county=data_crop(idx);
             Irr_nonIrr_county=temp_province.*crop_proportion_current_county;
...
%% save results
xlswrite([land_loc,num2str(year_map),'\county_level\test08\CIrrMap_area_sim_obs_county_',num2str(year_map),'_final.xlsx'],Irr_NonIrr_Area);

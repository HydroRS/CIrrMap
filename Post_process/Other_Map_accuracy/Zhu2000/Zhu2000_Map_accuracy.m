...
sutability_threshod_IrrArea= geotiffread([land_loc,num2str(year_map),'\county_level\Other_IrrMap\zhu_map2000.tif']);
...
for kk=1:31
    ...
    idx_map = sub2ind(size(China_county_crop_aridity),current_province_id(:,1),current_province_id(:,2)); % row/con to index
     ...
     sutability_province=sutability_threshod_IrrArea(idx_map)/100;
...
for kk=1:31
    ...
    idx_map = sub2ind(size(sutability_threshod_IrrArea),current_province_id(:,1),current_province_id(:,2)); % row/con to index
     ...
     sutability_province=sutability_threshod_IrrArea(idx_map);
       Irr_sutability=sutability_province(id(:,end),:)/100;
...
xlswrite([output_local,num2str(year_map),'\county_level\Other_IrrMap\zhu_map2000_Point_accuracy.xlsx'],acccuracy_China, 'acccuracy_China');
xlswrite([output_local,num2str(year_map),'\county_level\Other_IrrMap\zhu_map2000_Point_accuracy.xlsx'],accuarcy_Province, 'accuarcy_Province');

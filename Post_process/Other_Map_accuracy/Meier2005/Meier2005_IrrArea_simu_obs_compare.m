...
CIrrMap= geotiffread([land_loc,num2str(year_map),'\county_level\Other_IrrMap\Meier2005.tif']);
...
%            idx = sub2ind(size(data_crop),current_province_crop_id(crop_current_county,1),current_province_crop_id(crop_current_county,2)); % row/con to index
           crop_proportion_current_county=data_crop(idx);
             ...
             Irr_nonIrr_county=double(temp_province).*1; % 没有比例
...
%% save results
xlswrite([land_loc,num2str(year_map),'\county_level\Other_IrrMap\Meier2005_area_sim_obs_county_',num2str(year_map),'.xlsx'],Irr_NonIrr_Area(:,1:end-1));

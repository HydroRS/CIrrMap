...
for jj=1:31
    ...
    veg=[EVI_2000{jj}(:,1),NEVI_2000{jj}, NDVI_2000{jj}(:,1),NNDVI_2000{jj},GI_2000{jj}(:,1),NGI_2000{jj}];
    Env=env_variables{jj}(:,6);
...
%             predictor_data_all{kk,jj}=[veg(county_in_province,:),Env(county_in_province,:)];
            ...
            temp_in=[veg(all_sample,:),Env(all_sample,:)];
            if isempty(temp_in)
                continue
            end
            temp_out=label;
            predict_temp=[veg(county_in_province,:),Env(county_in_province,:)];
...
xlswrite([output_local,num2str(year_map),'\county_level\test08\IrrArea_map_point_accuracy_rng10.xlsx'],acccuracy_China,'acccuracy_China');
xlswrite([output_local,num2str(year_map),'\county_level\test08\IrrArea_map_point_accuracy_rng10.xlsx'],accuarcy_Province,'accuarcy_Province');

mapping_result_rng10=mapping_result;
save ([land_loc,num2str(year_map),'\county_level\test08\mapping_result_rng10.mat'], 'mapping_result_rng10','-v7.3');
%%
% geotiffwrite([land_loc,num2str(year_map),'\county_level\test08\',num2str(year_map),'_Irr_map_county_China_rng10.tif'],Irr_map_China,Ref, 'GeoKeyDirectoryTag', data_GeoInfor.GeoTIFFTags.GeoKeyDirectoryTag);

%% #####################RF Training and Predict################
% %  paramters
% paroptions = Parellel_set();
%
% leaf=10;
...
%
% weight_all=[];
% mapping_result=cell(size(predictor_data_all));
% Irr_map_China=double(zeros(size(data_crop)));
%
% for ii=22
%     ii
...
%         kk
%
%         temp_in=train_data_subset{kk,ii}(:,1:end-1);
%           if isempty(temp_in)
%             continue
%           end
%         temp_out=train_data_subset{kk,ii}(:,end);
%         predict_temp=predictor_data_all{kk,ii};
%
%
%            RF_model= TreeBagger(ntrees,temp_in,temp_out,'Method','classification','oobvarimp','on','surrogate',...
%                 surrogate,'minleaf',leaf,'FBoot',fboot, 'Options',paroptions);
